...
const run = async() =>{
    ...
    await contract.updateListPrice(10000000000);
    liPrice = await contract.getListingPrice();
    console.log("Updated price",Number(liPrice));
    var tokens = await contract.getAllTokens();
    console.log(tokens);
    tokens = await contract.getListedTokens();
    console.log(tokens);
    tokens = await contract.getMyTokens(owner.address);
    console.log(tokens);
}
